...
def EliminatePossiblePositions(dctPositions, strActNumber):
    ...
    """
    for i in xrange(len(strActNumber)):
...
def GenerateNewNumberFixedPart(lstNumbersFound, charMissing, dctPositions):
    ...
    for num in itertools.permutations(lstPermutInit,4):
        boolInnerCond = True
        for i in xrange(len(lstNumbersFound)):
            if lstNumbersFound[i] in [num[x] for x in xrange(4) if dctPositions[int(lstNumbersFound[i])][1][x]]:
...
def CombineFixedAndMovingPart(strFixedPart, strMovingPart, charMissing):
    cntX = 0
    strNewGuess = ""
    for pos in xrange(len(strFixedPart)):
...
def AddNewNumber(lstNumbersFound,dctPositions,strActNumber):
    ...
    if strActNumber not in lstNumbersFound:
        ...
        for i in xrange(len(dctPositions[int(strActNumber)][1])):
...
def ComputerGuessAlgorithm1(strNumToGuess,strInitialGuess):
    ...
    """
    dctPossiblePositions = dict((number, [None,[None for x in xrange(4)]]) for number in xrange(10))
    intNumberFound = 0
    lstNumbersFound = list()
    lstNumbersEliminated = list()
    lstGuesses = list()
...
while True:
    dctResults = dict()
    strNumToGuess =  "".join([str(x) for x in random.sample(xrange(10), 4)])
    ...
    UserGuessCount, UserCorrectGuessCount = UserGuess(strNumToGuess)
    if (UserGuessCount, UserCorrectGuessCount) == (-1,-1):
        print "\nYou have given up, You have lost against the computer"
    else:
        if UserCorrectGuessCount == UserCorrectGuessCount:
            print "\nYou have found the solution in {0} guesses.".format(UserGuessCount)
        else:
            print "\nYou have found the solution in {0} guesses, but only {1} guesses were in correct format".format(UserGuessCount,UserCorrectGuessCount)
        dctResults["You"] = UserCorrectGuessCount
        sorted_ranking = sorted(dctResults.items(), key=lambda x:x[1])
        intFillLength = max([len(x) for x in dctResults.keys()]) + 2
        
        
        print "\n{0}\nGame finished!\nRankings:\n{1}: {2}\t{3}\n{4}: {5}\t{6}\n{7}: {8}\t{9}\n{0}\n".format(
        StrHorizontal,1,sorted_ranking[0][0].ljust(intFillLength),sorted_ranking[0][1],2,sorted_ranking[1][0].ljust(intFillLength),sorted_ranking[1][1],
            3,sorted_ranking[2][0].ljust(intFillLength),sorted_ranking[2][1],StrHorizontal)
        
        print StrHorizontal
        if sorted_ranking[0][0] == "You":
            print "I am really impressed, You have beaten both computer algorithms"
        elif sorted_ranking[-1][0] == "You":
            print "You have lost!"
        else:
            print "Not so bad, at least You haven't finished last"
        print StrHorizontal
        
    
    print "\nHere are the steps used by the computer algorithms:"
    for algNr in xrange(len(lstComputerSolution)):
        for i in xrange(len(lstComputerSolution[algNr])):
            print "Player {0}: Guess nr. {1}: {2}, Bulls: {3}, Cows: {4}".format(algNr + 1,i+1,lstComputerSolution[algNr][i][0],
                lstComputerSolution[algNr][i][1],lstComputerSolution[algNr][i][2])
        print ""
    ExitPrompt = raw_input("One more round (yes or no)?: ")
    ExitPrompt = ExitPrompt.lower()
    if ExitPrompt == "n" or ExitPrompt == "no":
        exit()
    elif ExitPrompt == "y" or ExitPrompt == "yes":
        RoundNr += 1
        continue
    else:
        print "That is an incorrect answer. I am rather turning off the program"
        exit()
